...
def fit_transform(X_highdimensional, n_neighbors=5):
    '''
        Performs Hessian LLE on the given highdimensional dataset. The data needs to be a numpy array of the form (n_samples, n_features).
    ...
    '''
    hlle = manifold.LocallyLinearEmbedding(n_neighbors, n_components=2, method='hessian', eigen_solver="dense")
    t0 = time()
    X_low = hlle.fit_transform(X_highdimensional)
    return X_low, (time() - t0), hlle.reconstruction_error_