...
# LOAD DATA
X, Y = MnistLoader.load_mnist(n_samples=300)
...
def plot_low_dimensional_embedding(X_low, name="", time=0, error=0):
    ...
    for i in range(X.shape[0]):
        plt.text(X_low[i, 0], X_low[i, 1], str(Y[i]),
...
# Isomap
X_isomap, tisomap, err_isomap = Isomap.fit_transform(X, n_neighbors=275)
plot_low_dimensional_embedding(X_isomap, "Isomap", tisomap, err_isomap)
# Kernel PCA
X_kpca, tkpca = Kernel_PCA.fit_transform(X, t_kernel="poly")
plot_low_dimensional_embedding(X_kpca, "Kernel PCA", tkpca)
# Laplacian Eigenmaps
X_laplacian, tlaplacian = Laplacian_Eigenmaps.fit_transform(X, n_neighbors=275)
